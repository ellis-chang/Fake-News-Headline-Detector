...
class App extends Component {
  ...
  handlePredictClick = (event) => {
    ...
    //TESTING
    this.setState({ history: [...this.state.history, {name: formData.headline, result: false} ] });
    ...
    fetch('http://127.0.0.1:5000/prediction/', 
      ...
      .then(response => {
        this.setState({
          ...
          isLoading: false,
          // history: [...this.state.history, {name: formData.headline, result: response.result} ]
          
          //Testing
          // history: [...this.state.history, {name: formData.headline, result: "REAL"} ]
          ...
          {result === "" ? null :
            (<Row>
              <Col className="result-container">
                <h5 id="result">{`We think that the news is ${result}`}</h5>
              </Col>
            </Row>)
          }
          <Row>
